...
export const TrendingBlogs = () => {
    ...
    useEffect(() => {
        const fetchblogs = async () => {
            try {
                ...
                const sortedBlogs = [...res.data.data].sort((a, b) => {
    ...
    if(!BlogData[0]) return (<p className="text-center text-2xl flex mt-44 justify-center ">Loading....</p>)
        ...
        <div className="min-h-screen text-white px-8 py-12">
            <header className="text-center mb-12">
                ...
                <div className="mt-4 mb-6  bg-cyan-800/30 rounded-lg">
                    <ul className="flex flex-row p-2 justify-center space-x-40">
                    <li>
                            <button
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10">
                {BlogData.map((blog, index) => (
                    <div
                        key={index}
                        className="bg-gray-800/60 p-6 rounded-lg shadow-lg transition-transform transform hover:scale-105"
                    >
                        ...
                        <div className="flex items-center justify-between">
                            <span className="text-cyan-400 border-lg text-sm font-semibold">
                                {blog.tags.join(", ")}
                            </span>
                            <button className="text-sm text-black bg-yellow-500 hover:bg-yellow-800 px-3 py-1 rounded-full">
                                <Link to={`/blogdetail/${blog._id}`}>Read More</Link>
                            </button>
                        </div>
                    </div>
                ))}
